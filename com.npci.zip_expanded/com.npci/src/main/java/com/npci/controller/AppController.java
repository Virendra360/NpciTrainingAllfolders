...
//	 creating for to register a profile
    ...
    {
 	   Profile newpf=profileService.createProfile(pf.getName(),pf.getEmail_id(),pf.getPassword(),
 			   pf.getDob(),pf.getPhone());
 	   return ResponseEntity.status(200).body(newpf);
    }
    
    //webservice to get single profile
    @GetMapping("/{emailId}")
    public ResponseEntity<Profile>createNewProfile(@PathVariable("emailId") String email_id)
    {
  	   Profile newpf;
	try {
		newpf = profileService.showProfileByEmailId(email_id);
		return ResponseEntity.status(200).body(newpf);
	} catch (ProfileNotFoundException e) {
		System.out.println(e);
		exceptionList.add(e);
    ...
    // add contact webservice
    @PostMapping("/newContact")
    public ResponseEntity<Profile> addConact(@RequestBody Contact cnt)
    {
   	    return null;
    }
